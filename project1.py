...
def load_data():
    ...
    return X_train, y_train_cancer, X_train_cell, y_train_cell, X_val, y_val_cancer, y_val_cell, X_test, y_test_cancer, y_test_cell, test_paths, extra_data, dataset, scaler, pca
...
def predict_single_image(X_test, y_test_cancer, y_test_cell, test_paths, best_cancer_model, best_cell_model, dataset, scaler, pca):
    try:
        ...
        logger.info(f"Best isCancerous Model ({best_cancer_model.__class__.__name__}): Predicted = {pred_cancer}")
...
def main():
    # **LOAD DATA**
    X_train, y_train_cancer, X_train_cell, y_train_cell, X_val, y_val_cancer, y_val_cell, X_test, y_test_cancer, y_test_cell, test_paths, extra_data, dataset, scaler, pca = load_data()
    ...
    top_cancer_models = {k: v for k, v in models_cancer.items() if results_cancer[k]['val_f1'] > 0}
    ...
    top_cell_models = {k: v for k, v in models_cell.items() if results_cell[k]['val_f1'] > 0}
    ...
    ultimate_judgement = {
        'isCancerous': {
            ...
            'justification': "Selected for highest validation F1-score, leveraging advanced feature engineering and ensemble techniques for robust binary classification."
        },
        'cellType': {
            'best_model': best_cell_model_name,
            'metrics': results_cell[best_cell_model_name],
            'justification': "Chosen for balanced performance across four classes, enhanced by SMOTE, semi-supervised learning, and ensemble methods."
        },
        'comparison': {
            ...
            'analysis': "isCancerous classification achieves higher performance due to simpler binary task and robust feature extraction. Cell-type classification is improved with SMOTE and ensemble methods but remains challenging due to class imbalance."
        },
